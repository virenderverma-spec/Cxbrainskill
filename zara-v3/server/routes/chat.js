...
const { getLob } = require('../config/lob-registry');
...
## Drafting Emails for Agents

When an agent asks you to draft an email or reply to a customer, follow these guidelines from the reactive communication playbook:

### Structure (Multi-Issue Consolidated Response)
\`\`\`
...
{specific diagnosis, action taken, next steps}
...
### Priority Ordering for Issues in Email
...
### Channel Selection for Multi-Issue Responses
- Single issue, single channel → reply on same channel
- Multi-issue from email → email response
- Multi-issue from chat → detailed email + brief chat note: "I've sent you a detailed email covering everything"
- Multi-issue from social → detailed email + brief social reply: "I've sent a detailed response to your email"

### Email Tone Guidelines
- Lead with what you KNOW, not questions — use customer context data
- Be specific about what happened and what's next
- Include timelines when possible (e.g., "within the hour", "1-3 business days")
- If referencing a proactive outreach already sent, acknowledge it: "As mentioned in our earlier email..."
- Never say "we're working on it" without a specific next step or timeline
- Sign off with agent's name + "Meow Mobile Support"

### Single-Issue Email (Simpler Format)
\`\`\`
Hi {customer_name},

{acknowledge the issue — lead with what you see in their account}

{what happened / root cause}

{what we've done / what happens next + timeline}

{any action needed from the customer, if applicable}
...
/**
 * v3.1: Build dynamic system prompt with LOB overlay.
 * Always loads at least base-telco overlay; LOB-specific overlay adds brand context.
 */
function buildSystemPrompt(lobId) {
  let prompt = SYSTEM_PROMPT;

  // Always load base overlay; LOB-specific overlay overrides if available
  const lob = lobId ? getLob(lobId) : null;
  const overlayFile = (lob && lob.systemPromptOverlay) ? lob.systemPromptOverlay : 'base-telco.md';
  const overlayPath = path.join(__dirname, '..', '..', 'prompts', overlayFile);
  try {
    if (fs.existsSync(overlayPath)) {
      const overlay = fs.readFileSync(overlayPath, 'utf-8').trim();
      const overlayLabel = lob ? lob.displayName : 'General';
      prompt += `\n\n## LOB Context: ${overlayLabel}\n${overlay}`;
    }
  } catch (e) { /* ignore */ }

  if (lob) {
    // Replace hardcoded "Meow Mobile" with LOB display name
    prompt = prompt.replace(/Meow Mobile Support/g, `${lob.displayName} Support`);
  }
...
router.post('/', async (req, res) => {
  const { message, sessionId = 'default', customerContext, lobId } = req.body;
  ...
  try {
    ...
    for (let i = 0; i < 10; i++) {
      response = await anthropic.messages.create({
        ...
        max_tokens: 4096,
        system: buildSystemPrompt(lobId),
        tools: TOOLS,
        messages,
      });
